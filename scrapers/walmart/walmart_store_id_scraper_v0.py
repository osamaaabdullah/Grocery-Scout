...
def get_response(postal_code,session):
...
def scrape_store_info(postal_code,session):
    response = get_response(postal_code,session)
    try:
        data = response.json()
        return data
            ...
            time.sleep(120)
            return scrape_store_info(postal_code,session)

def parse_stores_json(stores, data):
    postal_list = []
