...
@router.post("/product")
async def upsert_product(product: ProductCreate, db: Session = Depends(get_write_db), current_user: Annotated[User, Depends(role_required("admin"))] = None):
...
@router.get("/products")
async def get_products(product_name: str = None, category: str = None, retailer: str = None, db: Session = Depends(get_read_db)):
    return product_services.get_products(db, product_name, category, retailer)

@router.get("/product/{product_id}")
async def get_product(product_id: str, db: Session = Depends(get_read_db)):
    return product_services.get_product_by_id(db, product_id)

@router.delete("/product/{product_id}")
async def delete_product(product_id: str, db: Session = Depends(get_write_db), current_user: Annotated[User, Depends(role_required("admin"))] = None):
