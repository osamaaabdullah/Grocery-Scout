...
async def forgot_password(email: PasswordResetEmail, db: Session = Depends(get_db)):
    load_dotenv()
    website_url = os.getenv("WEBSITE_URL")
...
async def reset_password(token:str, password: PasswordReset, db: Session = Depends(get_db)):
